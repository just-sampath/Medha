...
function sendDupli(){
  ...
  var sub = 'Your Registration for TREASURE HUNT has been Cancelled!!!'
  for(i=2;i<=to;i++)
  {
...
function sendMailREG() {
  ...
  let ran = fr+100;
  Logger.log(to-fr);
  Logger.log('The Last Row is '+ran);
  for(i=fr;i<=ran;i++){
      assignID(i);
      let bod = message(i);
